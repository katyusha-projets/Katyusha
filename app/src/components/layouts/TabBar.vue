...
<template>
  <Suspense>
    <div :class="`tab-bar-container ${modalStore.isAModalOpen ? 'modal-open' : ''}`">
      <nav class="tab-bar">
        <template v-for="i in config.layout.tabBar.items" :key="i.id">
          <div :class="`${modalStore.modalState[i.modalKey] ? 'active' : ''}`" @click="modalStore.open[i.modalKey]"><i :class="`fa-light fa-${i.icon}`" /><span>{{ i.title }}</span></div>
        </template>
      </nav>
